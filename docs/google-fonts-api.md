...
1. Add a stylesheet link to request the desired web font(s):
   ...
   ```css
   <link rel="stylesheet"
     href="https://fonts.googleapis.com/css?family=<var translate="no">Font+Name</var>">
   ```
...
2. Style an element with the requested web font, either in a stylesheet:
   ...
   ```css
   .css-selector {
   font-family: '<var translate="no">Font Name</var>', serif;
   }
   ```
...
[Inconsolata](https://fonts.google.com/specimen/Inconsolata), and
...
font's listing in [Google Fonts](https://fonts.google.com).
...
[font-display](https://www.w3.org/TR/css-fonts-4/#font-display-desc) lets you
...
Some of the fonts in the [Google Font Directory](https://fonts.google.com)
...
[Roboto Mono](https://fonts.google.com/specimen/Roboto+Mono) font, the URL would
be:
...
characters. For example, Â¡Hola! is represented as:
...
"[css text effects](https://www.google.com/search?q=css+text+effects)" and
browse through many of the ideas that are already on the web!
